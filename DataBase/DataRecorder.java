...
import AllClass.ClasseLivraria.LivrosSimples;
import AllClass.ClasseLivraria.Revistas;
import AllClass.ClasseDeUsuario.UsersLibrary;
...
public class DataRecorder {
	...
	public List<Livros> dataRecoverBooksInArchives() throws IOException {
		List<Livros> dataBooks = new ArrayList<>();
		...
		for (String line: textBooks) {
			...
			Livros b = new LivrosSimples(data[0], data[1], data[2], data[3],data[4],convertInt,convertDouble);
			dataBooks.add(b);
		}
		return dataBooks;
	}

	public List<Revistas> dataRecoverMagazinesInArchives() throws IOException {
		List<Revistas> dataMagazines = new ArrayList<>();
		...
		for (String line: textMagazines) {
			...
			Revistas m = new Revistas(data[0], data[1], data[2], data[3],convertDouble,convertInt);
			dataMagazines.add(m);
		}
		return dataMagazines;
	}

	public void recordMagazinesInArchives(List<Revistas> magazines) throws IOException {
		List<String> magazinesTxt = new ArrayList<>();
		for (Revistas m: magazines) {
	...
	public void recordBooksInArchives(List<Livros> books) throws IOException {
